...
function CircularProgress({ value, color, label, unit = '%' }: CircularProgressProps) {
  const r = 29;
  const strokeWidth = 2;
    ...
    <div className="relative flex items-center justify-center w-[76px] h-[76px] shrink-0 select-none">
      ...
      <div className="absolute flex flex-col items-center justify-center text-center">
        <span className="text-[15px] font-semibold text-white tracking-tight leading-none">
          {value.toFixed(0)}
          <span className="text-[10px] text-white/60 font-semibold ml-0.5">{unit}</span>
        </span>
      ...
      <div className="flex flex-col gap-1.5 w-[106px] shrink-0 justify-center h-full relative pl-1">
        {(['cpu', 'ram', 'gpu', 'storage'] as const).map((tab) => {
          ...
          const config = {
            ...
            ram: { label: 'RAM', icon: MemoryStick, value: stats.memory_percent },
            gpu: { label: 'GPU', icon: Zap, value: stats.gpu_usage },
            storage: { label: 'DISK', icon: HardDrive, value: stats.disks[0]?.percent ?? 0 },
          }[tab];
          ...
          return (
            <button
              ...
              className={`relative flex items-center justify-between px-2.5 py-1.5 rounded-[10px] text-[10px] font-semibold transition-all duration-200 cursor-pointer focus:outline-none select-none z-10 group ${
                isActive ? 'text-white font-bold' : 'text-white/45 hover:text-white/60'
              }`}
            >
              {isActive && (
                <motion.div
              ...
              <span className={`text-[10px] ${isActive ? 'text-[#0a84ff] font-extrabold' : 'text-white/50 font-bold'}`}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeSubTab === 'storage' && stats.disks.length > 1 ? (
              <div className="flex flex-col gap-3 w-full h-full justify-center py-0.5 overflow-hidden">
                {stats.disks.map((disk, idx) => (
                  <div key={idx} className="flex flex-col gap-1 w-full border-b border-white/[0.02] last:border-b-0 pb-1.5 last:pb-0">
                    <div className="flex justify-between items-baseline">
                      <span className="font-semibold text-white/95 text-[11.5px]">Drive ({disk.name})</span>
                      <span className="text-[10.5px] text-white/60 font-medium">
                        {formatDiskGB(disk.used)} GB of {formatDiskGB(disk.total)} GB ({disk.percent.toFixed(0)}%)
                      </span>
                    </div>
              ...
              <>
                ...
                <CircularProgress 
                  value={
                    activeSubTab === 'cpu' ? stats.cpu_usage :
                    activeSubTab === 'ram' ? stats.memory_percent :
                    activeSubTab === 'gpu' ? stats.gpu_usage :
                    stats.disks[0]?.percent ?? 0
                  } 
                  color="#0a84ff" 
                  label={
                    activeSubTab === 'cpu' ? 'util' :
                    activeSubTab === 'ram' ? 'alloc' :
                    activeSubTab === 'gpu' ? 'core' :
                    'used'
                  } 
                />
                ...
                <div className="flex-1 flex flex-col justify-center min-w-0 h-full py-1">
                  {activeSubTab === 'cpu' && (
                    <div className="flex flex-col justify-between h-full min-w-0">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-medium text-white/30 uppercase tracking-widest">Processor</span>
                        <span className="text-[13.5px] font-semibold text-white tracking-tight leading-tight truncate pr-1" title={stats.cpu_name}>
                          {stats.cpu_name}
                        </span>
                      </div>
                  ...
                  {activeSubTab === 'ram' && (
                    <div className="flex flex-col justify-between h-full min-w-0">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-medium text-white/30 uppercase tracking-widest">Memory</span>
                      ...
                      <div className="grid grid-cols-2 gap-x-4 mt-1.5 text-white/60">
                        <div className="flex flex-col">
                          <span className="text-[9px] font-bold text-white/30 uppercase tracking-wider">Used</span>
                          <span className="text-white/95 font-semibold text-[11.5px]">{formatGB(stats.used_memory)} GB</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[9px] font-bold text-white/30 uppercase tracking-wider">Total</span>
                          <span className="text-white/95 font-semibold text-[11.5px]">{formatGB(stats.total_memory)} GB</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {activeSubTab === 'gpu' && (
                    <div className="flex flex-col justify-between h-full min-w-0">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-medium text-white/30 uppercase tracking-widest">Graphics</span>
                        <span className="text-[13.5px] font-semibold text-white tracking-tight leading-tight truncate pr-1" title={stats.gpu_name}>
                          {stats.gpu_name}
                        </span>
                      </div>
                      <div className="flex items-center mt-1.5">
                  ...
                  {activeSubTab === 'storage' && stats.disks.length === 1 && (
                    <div className="flex flex-col justify-between h-full min-w-0">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-medium text-white/30 uppercase tracking-widest">Storage</span>
                        <span className="text-[13.5px] font-semibold text-white tracking-tight leading-tight">
                          Drive ({stats.disks[0].name})
                        </span>
                      </div>
                      <div className="grid grid-cols-2 gap-x-4 mt-1.5 text-white/60">
                        <div className="flex flex-col">
                          <span className="text-[9px] font-bold text-white/30 uppercase tracking-wider">Used</span>
                          <span className="text-white/95 font-semibold text-[11.5px]">{formatDiskGB(stats.disks[0].used)} GB</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[9px] font-bold text-white/30 uppercase tracking-wider">Available</span>
                          <span className="text-white/95 font-semibold text-[11.5px]">{formatDiskGB(stats.disks[0].total - stats.disks[0].used)} GB</span>
                        </div>
                      </div>
                    </div>
                  )}
