  ...
  useEffect(() => {
    const fetchRegistry = async () => {
      ...
      try {
        ...
        console.error("[PluginsTab] Failed to fetch registry:", e);
        setRegistryError("Failed to load store plugins.");
      } finally {
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        ...
        {loadingRegistry ? (
          ...
          <div className="text-center text-xs text-zinc-500 py-8 bg-black/20 rounded-lg border border-white/10">
            {registryError}
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {registryPlugins.map((reg) => {
              ...
              return (
                <div
                  key={reg.id}
                  className="bg-black/20 rounded-lg border border-white/10 p-4 flex items-center justify-between gap-4"
                >
                  ...
                  <button
                    ...
                    onClick={() => installPlugin(reg.manifestUrl)}
                    className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors flex-shrink-0 active:scale-[0.98] ${
                      isInstalled
