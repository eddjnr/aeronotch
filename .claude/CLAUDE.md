...
## IMPORTANT: Codebase Intelligence Instructions for winotch
...
Last indexed: 2026-07-05 (commit ebfe36e). Confidence: 100%.
### Architecture
repo bridges Rust-based Tauri system commands w/ TypeScript/React frontend to orchestrate window management, state persistence, & localized UI rendering.
### Entry Points
- `src/App.tsx`
- `src/main.tsx`
...
| Layer | Files | Purpose |
|-------|-------|---------|
| Interface Component Library | 18 | A collection of reusable React components and interactive widgets that form the… |
| System Integration Logic | 34 | A bridge between frontend hooks and backend system modules that manages… |
| Project Configuration Assets | 13 | A collection of environment settings, development workflows, and agent-specific… |
| Global State Management | 3 | Centralized stores that manage and persist the application's global state… |
| Schema Definition Files | 5 | A set of type definitions and JSON schemas that define the structure and… |
| Frontend Utility Layer | 4 | Provides shared helper functions, system command interfaces, and global… |
| Release Automation Scripts | 2 | Automates versioning and changelog generation tasks to streamline the project… |

### Guided Tour (11 steps)
1. `README.md` — Start here for the end-to-end picture before diving into the code.
2. `src/App.tsx` — An entry point — execution and imports fan out from here.
3. `src/main.tsx` — An entry point — execution and imports fan out from here.
4. `src-tauri/src/main.rs` — An entry point — execution and imports fan out from here.
5. `src-tauri/src/lib.rs` — An entry point — execution and imports fan out from here.
6. `src/lib/tauri-commands.ts` — Directly used by the entry points above; a core collaborator.
... and 5 more steps
### Hotspots (High Churn)
| File | Churn | 90d Commits | Owner |
|------|-------|-------------|-------|
| `src-tauri/src/google_calendar.rs` | 99.2th %ile | 11 | Junior Albuquerque |
| `src/components/island/IslandLayout.tsx` | 98.4th %ile | 14 | Junior Albuquerque |
| `src/components/settings/SettingsPanel.tsx` | 97.6th %ile | 31 | Junior Albuquerque |
| `src-tauri/src/lib.rs` | 96.8th %ile | 13 | Junior Albuquerque |
| `src-tauri/src/system_info.rs` | 96.0th %ile | 8 | Junior Albuquerque |
...
Defect risk, Hotspot health: 3.98/10 (stable) ·
Average: 8.73/10 ·
Worst: 1.65/10 (`src-tauri/src/google_calendar.rs`)
Maintainability, Average: 9.32/10
Performance risk, Average: 9.98/10

### Critical biomarkers
- `src/components/island/IslandLayout.tsx` — nested complexity (useEffect callback) — impact −2.5
- `src/components/widgets/CalendarWidget.tsx` — prior defect — impact −2.0
- `src/components/island/Island.tsx` — function hotspot (Island) — impact −1.6
- `src-tauri/src/media.rs` — nested complexity (build_media_info) — impact −1.1
- `src/i18n/translations.ts` — change entropy — impact −1.0
